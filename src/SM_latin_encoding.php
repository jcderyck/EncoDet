...
//	Returns array (string encoding, float confidence)
	...
	foreach ($encodings as $key=>$encoding) {
		...
		if (!$stringUTF) {unset($encodings[$key]); continue;}

		// get rid of redundant encodings
		$string_hash = md5($stringUTF);
		...
		$result1 = preg_match_all('~\w[^´‘’—\x{C2A0}\w\x00-\x7F]\w~u', $stringUTF, $words);
		...
		$result1 = preg_match_all('~[^\P{Lu}I][^\P{Lu}A-Z][^\P{Ll}l]~u', $stringUTF, $words);
		$results[$encoding] -= 0.01*$result1;
		$faults[$encoding] += $result1;
		if ($faults[$encoding] > 10) {unset($encodings[$key]); continue;}

		// lowercase letters followed by uppercase
		$result1 = preg_match_all('~[a-z][^\P{Lu}A-Z]~u', $stringUTF, $words);
		$result1 += preg_match_all('~[^\P{Ll}a-z][^\P{Lu}I]~u', $stringUTF, $words);
		$results[$encoding] -= 0.01*$result1;
		$faults[$encoding] += $result1;
		if ($faults[$encoding] > 10) {unset($encodings[$key]); continue;}

		// number of upper byte control codes
		$result1 = preg_match_all('~[\x{0080}-\x{009F}]~u', $stringUTF, $words);
		$results[$encoding] -= 0.01*$result1;
		$faults[$encoding] += $result1;
		if ($faults[$encoding] > 10) {unset($encodings[$key]); continue;}

		// number of common symbols
		$results[$encoding] += preg_match_all('~[´‘’—\x{C2A0}]~u', $stringUTF, $words);

		// number of title words at least 2 letters, assumed to be proper nouns
		$results[$encoding] += 0.99*preg_match_all('~\b[A-Zl][Ia-z]*[^\P{Ll}a-z][\p{Ll}I]*\b~u', $stringUTF, $words1); // Aä
		$results[$encoding] += 0.99*preg_match_all('~\b[^\P{Lu}A-Z][\p{Ll}I]+\b~u', $stringUTF, $words2); // Äa Ää
...
//		$results[$encoding] += 0.01*count(preg_filter($common_words, "", $words2[0])); // uncomment if common words starting with diacritics added
		...
		$results[$encoding] += 0.99*preg_match_all($pattern, $stringUTF, $words);
		...
		preg_match_all($pattern, $stringUTF, $words);
		$words = str_replace('I', 'l', $words[0]);
		...
		preg_match_all('~.[“„«–…€£±©¶¤][^\x0D\x0A]{3}~su', $stringUTF, $words);
		$matches = preg_filter($common_symbols[1], "$0", $words[0]);
		$results[$encoding] += 0.99*count($matches);

		preg_match_all('~[^\x0D\x0A]{3}[”»–°º¼½¾‰€¶¤].~su', $stringUTF, $words);
		$matches = preg_filter($common_symbols[2], "$0", $words[0]);
		$results[$encoding] += 0.99*count($matches);
